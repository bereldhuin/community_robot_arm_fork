...
//   V0.91 SERIAL2 + GRIPPER OPEN DEGREE
...
#if ENABLE_SERIAL2
      Serial2.println(PRINT_REPLY_MSG);
#endif
    }
  }
...
#if GRIPPER == SERVO      
      case 4:
        servo_gripper.cmdOpen(cmd.valueS);
        break;
#endif
      case 5:
#if GRIPPER == BYJ
...
void ps4_controller_loop() {
  ...
  if (controller_ps4.buttons[PS4_CROSS]) {
    if (GRIPPER == SERVO) { servo_gripper.cmdOn(); }
  }
  if (controller_ps4.buttons[PS4_CIRCLE]) {
    if (GRIPPER == SERVO) { servo_gripper.cmdOff(); }
  }
